...
**项目名称**：AI睡眠伙伴 (LTBot Next.js)  
**项目类型**：AI 驱动的故事与音乐创作平台  
**技术栈**：Next.js 16 + React 18 + TypeScript + Prisma + SQLite + Clerk + DeepSeek AI  
**当前版本**：v0.2.0  
**最后更新**：2026-02-02

### 故事深度定制化一期（2026-09-05）

- ✅ Prisma 加法迁移：`StoryVisibility`、`ChildProfile`、`StoryCustomization`；历史故事回填 `PUBLIC`，新故事默认 `PRIVATE`。
- ✅ 服务端校验与访问策略：Clerk 归属校验、私密故事 404 边界、本地内容风险规则、幂等创建、服务端生成 Prompt。
- ✅ 档案 API：创建、编辑、软删除、恢复、完成故事数统计。
- ✅ 三步创作、预设头像、私密生成结果回执、定制摘要、档案筛选页面。
- ✅ 档案弹框与创作页移动端操作栏修正；性格方向扩展为 8 项优秀品质（仍限选 1–3 项）。
- ✅ “我的故事”档案筛选按高保真原型改为可直接切换的圆角标签组，支持已删除档案状态文案。
- ✅ 孩子档案列表页层级收紧：页面标题恢复常规字号、删除重复说明，编辑/删除统一收到档案卡片右下角。
- ✅ 场景卡片功能 v0.1.0：第 2 步按四大类纵向展示，每类 6 张本地图片卡横向滑动；全局单选、分龄详情、返回恢复、键盘/箭头、图片降级和埋点已完成。
- ⏳ 生产环境 Clerk/DeepSeek 配置后的联调与桌面/移动端验收。
...
待完成功能：
- Clerk 认证集成：集成微信登录、注册；抖音
- 登录页面：/sign-in
- 注册页面：/sign-up
- 用户信息同步：POST /api/users/sync，生产环境同步有些问题
- Webhook 集成：POST /api/webhooks/clerk
- 中间件路由保护：middleware.ts
- useUserSync Hook：自动同步用户信息
- UserSyncProvider：全局用户同步组件
