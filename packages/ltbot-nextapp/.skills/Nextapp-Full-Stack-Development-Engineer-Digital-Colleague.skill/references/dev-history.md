...
## 2026-09-18 领养并入新建档案 + 花园双影修复与喂养动效（最新）

- 类型：交互与视觉修复。用户反馈两点：① 领养应回到新建档案流程，交互对齐场景卡横滑；② 浮岛花园同时露出站立/睡觉两态，喂养动画偏硬。
- 根因：`PetSprite` 使用 2×2 精灵图，花园 `.habit-pet` 容器非正方形且 CSS 覆盖了 sheet 尺寸，竖向窗口裁切不足，同时露出上下两格。
- 做了什么：新建档案增加必选宠物横滑轨（`PetAdoptionRail`，snap/箭头/键盘对齐 `DreamPlace`）；创建接口支持同事务写入 `petKey`；独立领养页改用同一横滑组件。`PetSprite` 强制 `aspect-ratio:1`，花园宠物改为正方形并保护 sheet 样式；喂养改为更长的食物弧线 + 弹性欢呼 + 结果弹出，请求时机错开约 420ms。
- 涉及文件：`src/components/pets/PetSprite.tsx`、`PetAdoptionRail.tsx`、`src/app/to-view-mine/child-profiles/page.tsx`、`src/app/api/child-profiles/route.ts`、`src/app/habits/adopt/page.tsx`、`companion/page.tsx`、`habits.css`、本开发档案。
- 验证：定向 TypeScript / ESLint 通过。浏览器实机喂养与新建档案横滑待登录态验收。
- 追加：隐藏旧成长/旧档案入口（打卡页、花园、领养页、孩子档案已删除区），不再展示旧数据导航。

## 2026-09-18 儿童打卡宠物化一期与定制故事联动实施

- 依据：产品本轮四项答复及已批准实施计划；无新版原型，继续严格参考打卡高保真原型 v0.4.0 的布局与交互骨架。
- 数据：新增版本化 12 种宠物目录、孩子唯一宠物、独立成长/库存/喂养/里程碑，以及故事宠物快照、成功序号和累计计数。旧伙伴成长与卡片保留只读；旧成功定制故事只按数据库内仍保留的正文建立基线。迁移已在临时库演练并应用本地 `prisma/dev.db`，备份在系统临时目录。
- 打卡与宠物：无宠物的新打卡被服务端阻止；领养后新宠物从零成长，旧完成位与奖励位继续占用。打卡记录本身有新旧标记，新打卡即使未获卡也可撤销，旧打卡只读。新卡/喂养不接触旧库存，首次成功喂养锁定物种，冲正不解锁。成长阶段改为 10/30/70/140，仅由成长值判定；最高阶段与里程碑保留。花园、卡片和动效沿旧原型，只增加领养页、归档入口与 12 种透明四态角色素材。
- 故事：定制故事增加默认开启的宠物选项，关闭无需领养，创作服务不再读取旧伙伴字段；开启时事务内冻结服务端宠物身份、阶段和最多 3 条白名单事实，草稿可在领养跳转后恢复。生成正文会校验宠物名字与物种，漏写追加一次重试；成功保存时故事、孩子累计数和宠物共同探索数原子幂等提交，失败与删除不扣减。结果和详情按快照展示；旧故事仍按旧摘要查看。
- 验证：`prisma validate`、临时库迁移、`pnpm typecheck`、定向 ESLint、Vitest 20 项、`pnpm build` 通过；Playwright 使用本机 Chrome 完成 9 项、跳过 7 项桌面不适用场景。全局 `pnpm lint` 在其他既有模块仍报 59 error / 33 warning；当前无登录会话，领养/喂养的真实浏览器主流程尚未验收；Playwright 专用 WebKit 浏览器下载被网络阻断，iPhone WebKit 回归待补。
- 详见：`docs/需求评审记录.md`、`docs/宠物化与故事联动接口.md`、`prisma/migrations/20260918130000_add_child_pets_and_story_success/`、`prisma/migrations/20260918143000_mark_pet_era_check_ins/`、`public/habits/pets/v1/`。

## 1. 2026-09 深度定制化评审期

### 2026-09-18 儿童打卡一期宠物化与故事联动优化 PRD v0.1.0 评审

- 类型：需求评审与技术决策准备。对照增量 PRD、基础 PRD 正文 v0.5.0 和当前孩子档案、习惯养成、故事创建/生成实现，评估一期统一主宠物与最小故事连续性。
- 结论：产品方向有条件通过，暂不作为开发/上线基线。P0 涉及旧档案身份迁移和未领养入口、故事覆盖范围/快照与成功计数、新成长阶段保值、12 种素材或首发子集放行口径，以及旧客户端和生成中任务的兼容回滚。
- 工程事实：花园目前会从旧 `partnerJson` 自动创建 `CompanionGrowth`（无映射则默认兔），现有外观接口允许持续更换；当前成长门槛为 20/60/140/300 且要求四类营养；定制故事创建即增加 `sequenceCounter`，`childOverrides` 仍可传伙伴；quick/continuation 故事允许无 `childProfileId`；现有宠物素材只有 5 张单态形象。
- 关键建议：产品先给出迁移决策表、未领养分步权限、故事统计口径和素材交付清单；技术随后做只读数据分布统计、保值迁移演练、不可变故事快照与服务端旧写口约束。旧数据和历史故事不得静默改写。
- 涉及文件：`.skills/Nextapp-Full-Stack-Development-Engineer-Digital-Colleague.skill/docs/需求评审记录.md`、本开发档案及生成的 `docs/dev-history.html`。本轮不修改业务代码。
- 验证：核对 PRD 对应章节、Prisma Schema、领域服务、API 和页面当前行为；评审记录已写入。无代码变更，不进行业务测试。
