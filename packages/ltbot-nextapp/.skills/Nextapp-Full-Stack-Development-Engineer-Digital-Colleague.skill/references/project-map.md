...
> 用途：每次开工前校准项目现状。维护者：Nextapp 全栈数字员工。最后更新：2026-09-11。
...
| `ChildProfile` | 孩子档案 | 预设头像（末位为自定义）、昵称、年龄段、角色（预设 id 或自定义自由文本）、8 项性格方向（限选 1–3 项）/伙伴 JSON；支持软删除与序号计数；历史 `bear` 头像只读兼容 |
