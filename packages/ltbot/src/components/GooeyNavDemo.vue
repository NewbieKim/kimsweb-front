...
// 泛型约束
// withDefaults：主要用于为props提供默认值、类型安全、响应式
...
//     {
//       items: {
//         type: Array as PropType<GooeyNavItem[]>,
//         default: () => [],
//       },
//       animationTime: {
//         type: Number,
...
// 用于创建响应式的基本类型或对象，访问和修改时需要使用 .value。
...
const makeParticles = (element: HTMLElement) => {
  ...
  for (let i = 0; i < props.particleCount; i++) {
    ...
    const p = createParticle(i, t, d, r); // 每个粒子的起始位置、结束位置、时间、缩放比例、颜色、旋转角度
...
const updateEffectPosition = (element: HTMLElement) => {
  ...
  textRef.value.innerText = element.innerText; // 把当前li的文字赋值给textRef
...
.effect.filter {
  ...
  /* 设置了混合模式为"lighten"，会使元素与其背景以变亮的方式混合 */
}
